...
try:
    ...
    from .operations import get_fnat_stats, residue_distances
    from .parsers import MMCIFParser, PDBParser
except ImportError:
    logging.warning(
        """Invoking DockQ as script rather than binary. This will slow down computations."""
    )
    from .constants import *
    from .operations_nocy import get_fnat_stats, residue_distances
    from .parsers import MMCIFParser, PDBParser
...
def calc_DockQ(
    ...
    nat_total = np.nonzero(np.asarray(ref_res_distances) < fnat_threshold**2)[0].shape[
        0
    ]
    ...
    assert sample_res_distances.shape == ref_res_distances.shape, (
    ...
    info["clashes"] = np.nonzero(np.asarray(sample_res_distances) < CLASH_THRESHOLD**2)[
        0
    ].shape[0]
