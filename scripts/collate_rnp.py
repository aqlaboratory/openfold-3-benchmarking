...
parser.add_argument(
    "--annot_df", type=str, required=True, help="Path to annotation CSV file"
)
parser.add_argument(
    "--method_name",
    type=str,
...
annot_df = pd.read_csv(args.annot_df, header=0)

ligand_to_proper = {}
ligand_to_sucos_shape = {}
ligand_to_protein_fident_weighted_sum = {}
ligand_to_topological_tanimoto = {}
ligand_to_morgan_tanimoto = {}
ligand_to_sucos_shape_pocket_qcov = {}
for _, row in annot_df.iterrows():
    ...
    for d in [
        ...
        if system_id not in d:
            d[system_id] = {}
    ligand_to_proper[system_id][row["ligand_instance_chain"]] = row["ligand_is_proper"]
    ligand_to_sucos_shape[system_id][row["ligand_instance_chain"]] = row["sucos_shape"]
    ligand_to_protein_fident_weighted_sum[system_id][row["ligand_instance_chain"]] = (
        row["protein_fident_weighted_sum"]
    )
    ligand_to_topological_tanimoto[system_id][row["ligand_instance_chain"]] = row[
        "topological_tanimoto"
    ]
    ligand_to_morgan_tanimoto[system_id][row["ligand_instance_chain"]] = row[
        "morgan_tanimoto"
    ]
    ligand_to_sucos_shape_pocket_qcov[system_id][row["ligand_instance_chain"]] = row[
...
for ost_result_json in tqdm(result_files, total=len(result_files)):
    query_name = ost_result_json.stem.split("_seed")[0]
    seed, sample = itemgetter(-3, -1)(ost_result_json.stem.split("_"))
    ...
    df_combined["ligand_is_proper"] = (
        df_combined["ligand_instance_chain"]
        .str[:-3]
        .map(ligand_to_proper.get(query_name, {}))
    )
    df_combined["sucos_shape"] = (
        df_combined["ligand_instance_chain"]
        .str[:-3]
        .map(ligand_to_sucos_shape.get(query_name, {}))
    )
    df_combined["protein_fident_weighted_sum"] = (
        df_combined["ligand_instance_chain"]
        .str[:-3]
        .map(ligand_to_protein_fident_weighted_sum.get(query_name, {}))
    )
    df_combined["topological_tanimoto"] = (
        df_combined["ligand_instance_chain"]
        .str[:-3]
        .map(ligand_to_topological_tanimoto.get(query_name, {}))
    )
    df_combined["morgan_tanimoto"] = (
        df_combined["ligand_instance_chain"]
        .str[:-3]
        .map(ligand_to_morgan_tanimoto.get(query_name, {}))
    )
    df_combined["sucos_shape_pocket_qcov"] = (
        df_combined["ligand_instance_chain"]
        .str[:-3]
        .map(ligand_to_sucos_shape_pocket_qcov.get(query_name, {}))
    )
